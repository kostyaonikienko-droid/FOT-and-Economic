...
class AnalyticsWidget(QWidget):
    ...
    def update_structure(self, orders):
        ...
        # Защита от NaN и нулевой суммы
        total = sum(v for v in values if not np.isnan(v))
        if total == 0:
            ax.text(0.5, 0.5, "Нет данных для отображения", ha='center', va='center', transform=ax.transAxes)
            ax.set_title("Структура себестоимости (нет данных)")
        else:
            values_clean = [0 if np.isnan(v) else v for v in values]
            ax.pie(values_clean, labels=labels, autopct='%1.1f%%', startangle=90)
            ax.axis('equal')
